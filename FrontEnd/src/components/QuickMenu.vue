<template lang="pug"> 
.quick-menu(ref='quickMenu', :style='quickMenuStyle', @click='closeAllMenu')
  .sub-menu(v-for='n in menuCount', :style='getSubMenu(n-1)')
    a(@click='launchAction(n-1)', :style='subMenuStyle', @mouseover.stop='mouseEnterSubMenu', @mouseout.stop='mouseOutSubMenu')
      i(:class='iconClass[n-1]', ref='icon')
  .menu(:style='menuStyle')
    .core-menu(@click='toggleMenu')
      .bar
</template>
<script>
	export default{
name:'quickMenu',
  ...
  methods:{
    launchAction(n){
      Event.$emit('fire_active_menu',(n))
      console.log('##__EMIT ->  active_menu : ' + n +'__##')
      this.toggleMenu();
      return 
    },
    getSubMenu(n){
    ...
    closeAllMenu(e){
      ...
      if (e.target.className === 'core-menu'){
        Event.$emit('fireCloseAllMenu');
        console.error("fireCloseAllMenu");
      }
